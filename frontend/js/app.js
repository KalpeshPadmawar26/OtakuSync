const BACKEND_URL = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') ? 'http://localhost:5000' : 'https://YOUR_PROJECT_NAME.onrender.com';
...
class App {
    ...
    createAnimeCard(anime, reason = "", isUpcoming = false) {
        ...
        const topBadge = anime.score ? `<div class="card-rating-badge">⭐ ${anime.score}</div>` : '';
        const genreHtml = anime.genres.slice(0, 2).map(g => `<span class="card-genre-badge">${g}</span>`).join('');
